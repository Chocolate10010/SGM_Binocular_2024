...
bool SemiGlobalMatching::Initialize(const uint32& width, const uint32& height, const SGMOption& option)
{
    width_  = width ; // 影像尺寸
    height_ = height;
    option_ = option; // SGM参数

    if(width == 0 || height == 0) {
        return false;
    }

    const sint32 img_size = width * height                                  ; // census值
...
bool SemiGlobalMatching::Reset(const uint32& width, const uint32& height, const SGMOption& option)
{
    ...
    return Initialize(width, height, option); // 初始化
}
...
bool SemiGlobalMatching::Match(const uint8* img_left, const uint8* img_right, float32* disp_left)
{
    ...
    if (option_.is_remove_speckles) {
        sgm_util::RemoveSpeckles(disp_left_, width_, height_, 1, 
                                 option_.min_speckle_aera, Invalid_Float)           ; // 移除小连通区
    }

    sgm_util::MedianFilter(disp_left_, disp_left_, width_, height_, 3)              ; // 中值滤波
    ...
    memcpy(disp_left, disp_left_, width_ * height_ * sizeof(float32))               ; // 输出视差图
        ...
        for (sint32 j = 0; j < width_; j++) {
        	for (sint32 d = min_disparity; d < max_disparity; d++) {
                ...
                if (option_.census_size == Census5x5) {
                    const auto& census_val_l = static_cast<uint32*>(census_left_)[i * width_ + j]       ; // 左影像census值
                    const auto& census_val_r = static_cast<uint32*>(census_right_)[i * width_ + j - d]  ; // 右影像对应像点的census值
                    cost = sgm_util::Hamming32(census_val_l, census_val_r)                              ; // 计算匹配代价
                }
        		else {
...
void SemiGlobalMatching::ComputeDisparity() const
{
    ...
    const auto disparity = disp_left_                           ; // 左影像视差图
	const auto cost_ptr = cost_aggr_                            ; // 左影像聚合代价数组
    ...
    std::vector<uint16> cost_local(disp_range)                  ; // 为了加快读取效率，把单个像素的所有代价值存储到局部数组里
    
    for (sint32 i = 0; i < height; i++) {                         // 逐像素计算最优视差
        for (sint32 j = 0; j < width; j++) {
            ...
            for (sint32 d = min_disparity; d < max_disparity; d++) {
                ...
                // const auto& 注意
                const auto& cost = cost_local[d_idx] = cost_ptr[i * width * disp_range +
                                                        j * disp_range + d_idx]             ;
                if( min_cost > cost) {
                    min_cost = cost                                         ;
                    best_disparity = d                                      ;
            ...
            if (is_check_unique) {                                   // 再遍历一次，输出次最小代价值
                for (sint32 d = min_disparity; d < max_disparity; d++) {
                    if (d == best_disparity) {
                        continue                                            ; // 跳过最小代价值
                    }
...
void SemiGlobalMatching::ComputeDisparityRight() const
{
    ...
    const auto disparity = disp_right_                      ; // 右影像视差图
	const auto cost_ptr  = cost_aggr_                       ; // 左影像聚合代价数组
