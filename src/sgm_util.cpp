 ...
 * \param p1		输入，代价参数P1
 * \param p2_init	输入，代价参数P2
 * \param cost_init	输入，初始代价
 * \param cost_aggr	输出，聚合代价
 * \param is_forward	输入，方向
 */
	...
	const sint32& p1, const sint32& p2_init, const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	...
	for (sint32 i = 0u; i < height; i++) {
		...
		//初始化路径上上个像素的代价数组：多两个元素是为了避免边界溢出（首尾各多一个）
		std::vector<uint8> cost_last_path(disp_range + 2, UINT8_MAX);

		//初始化聚合代价值：第一个像素的聚合代价值等于初始代价值
		...
		for (sint32 j = 0; j < width - 1; j++) {
			gray = *img_row;	//第二个像素的灰度值
			uint8 min_cost = UINT8_MAX;	//初值为255
			//遍历所有的视差值
			for (sint32 d = 0; d < disp_range; d++){
				// Lr(p,d) = C(p,d) + min( Lr(p-r,d), Lr(p-r,d-1) + P1, Lr(p-r,d+1) + P1, min(Lr(p-r))+P2 ) - min(Lr(p-r))
				const uint8  cost = cost_init_row[d];
				const uint16 l1 = cost_last_path[d + 1];		//[上个]像素，视差为d
